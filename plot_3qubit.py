...
ylims=[0.8,0.8,0.9,0.9]
xlims_right=[0.2,0.15,0.15,0.08]
lw =1.5
mv=[500,150,150,150]
ms=9
for (plt_index,K,jj,number1,number2) in zip(range(1,5),KK[0:4],JJ[0:4],L[0:4],U[0:4]):
    ...
    for i, color in zip(range(nb_classes), colors):
        ...
        ax.plot(fpr2[i], tpr2[i], color=color,linestyle=':', lw=lw,
        ...
        axins.plot(fpr2[i], tpr2[i], color=color,linestyle=':', lw=lw,marker='o',markersize=ms-2, markerfacecolor='none',markevery=mv[i+1])
        # 设置放大区间
        zone_left = 0
