...
for (number1,number2) in zip(Number1,Number2):
    ...
    history = pmodel.fit(label_datape,label, epochs=epoch ,validation_split=0.2,shuffle=True ,batch_size = int(number1/10))
    tra_acc, val_acc = history.history['accuracy'], history.history['val_accuracy']
'''
    ...
    for jj in range(iteration): 
        ...
        mmodel.fit(X,Y, epochs=Epoch,validation_split=0.2 ,batch_size = len(Y))
