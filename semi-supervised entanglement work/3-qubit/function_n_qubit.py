...
class generate():
    def __init__(self,number1,number2,n):
        ...
        self.step=0.0005
    def Ghz(self,N=None,v=0,b=1): 
        if N  is None:
            N=self.N
        la,ratio={},{}
        for i in range(int((self.number1/self.n))):
            for j in range(self.n):
                if v==0:
                    if j==(self.n-2):
                        #g=random.uniform(N[j],N[j+1]-b*((N[j+1]-N[j])/self.n))
                        #print("j",j)
                        #print("N[j+1]",N[j+1]-3*((N[j+1]-N[j])/self.n))
                    #elif j==(self.n-2):
                        g=random.uniform(N[j]+b*((N[j+1]-N[j])/self.n),N[j+1])
                        #print("j",j)
                        #print("N[j]",N[j]+3*((N[j+1]-N[j])/self.n))
                    else:
                        g=random.uniform(N[j],N[j+1])
                else:
                    g=random.uniform(N[j],N[j+1])
                ghz=((1-g)*self.I/(2**self.n)+g*(np.dot(self.ghz_n,(self.ghz_n.conj().T)))).reshape(1,2**self.n,2**self.n)
                if i==0:
                    self.GHZ["{0}".format(j+1)]=ghz
                else:
                    self.GHZ["{0}".format(j+1)]=np.concatenate((self.GHZ["{0}".format(j+1)],ghz), axis=0)
    
        for j in range(self.n):
            #print(len(self.GHZ["{0}".format(j+1)]))
            la["{0}".format(j+1)]=np.zeros((len(self.GHZ["{0}".format(j+1)]),self.n))
    ...
    def calculate_boundary(self,bpk): # input prediction of GHZ states
        bpk2=to_categorical(bpk, num_classes=self.n)
        i,j,bound_value=0,1,[]
        while j!=(self.n+1):
            ...
            if len(A)==0:
                ...
                bound_value.append(self.step*A[i])
        return bound_value
...
class get_data(generate):
    ...
    def Un_State(self):
        ...
        for j in range(self.n-1):
            if j==0:
                ...
                sea["{0}".format(j+1)]=Augmentation_Strategies(data=self.GHZ["{0}".format(j+1)],n=self.n).Convex_Combination(B=(self.n-j)+self.ratio["{0}".format(j+1)])
            
            if j==0: 
                UnD=sea["{0}".format(j+1)]
            else:
                UnD=np.concatenate((UnD,sea["{0}".format(j+1)]), axis=0)
    ...
    def test_Ghz(self,number3):
        GHZ_test,test_label,r=generate(number1=number3,number2=0,n=self.n).Ghz(v=1)
        test_ghz={}
        for j in range(self.n):
